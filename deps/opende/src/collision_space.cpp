...
void dxHashSpace::collide (void *_data, dNearCallback *callback)
{
  ...
  /******************************************
  * brief: try to use openmp to parallel the collision pairs collection
  * auther: chenjinghang
  * time: 2017.12.08
  * alter: 1. add #pragma omp parallel for statement.  2. add /gazebo/common/Timer.hh to catulate elapsed time
  */
  ///////////////////////////////////////////
  //common::Timer timer;
...
*/
  int ni = 0;
  dxAABB **aabb2_arr = (dxAABB **) ALLOCA (sizeof(dxAABB *) * (n));
  for(ni=0, aabb2=first_aabb; aabb2; ni++, aabb2=aabb2->next){
    aabb2_arr[ni] = aabb2;
  }
  ...
  for (ni=0; ni<n; ni++)
  {
    ...
    for (i=0; i<6; i++) db[i] = aabb2_arr[ni]->dbounds[i];
    for (int level = aabb2_arr[ni]->level; level <= maxlevel; level++) {
        ...
        for (int yi = db[2]; yi <= db[3]; yi++) {
          for (int zi = db[4]; zi <= db[5]; zi++) {
            ...
            for (node = table[hi]; node; node=node->next) {
              ...
              if (node->aabb == aabb2_arr[ni]) continue;
              if (node->aabb->level == level &&
                  node->x == xi && node->y == yi && node->z == zi) {
                // see if aabb and node->aabb have already been tested
                // against each other
                unsigned char mask;
                if (aabb2_arr[ni]->index <= node->aabb->index) {
                  i = (aabb2_arr[ni]->index * tested_rowsize)+(node->aabb->index >> 3);
                  mask = 1 << (node->aabb->index & 7);
                }
                else {
                  i = (node->aabb->index * tested_rowsize)+(aabb2_arr[ni]->index >> 3);
                  mask = 1 << (aabb2_arr[ni]->index & 7);
                }
                dIASSERT (i >= 0 && i < (tested_rowsize*n));
                if ((tested[i] & mask)==0) {
                  collideAABBs (aabb2_arr[ni]->geom,node->aabb->geom,_data,callback);
                }
                tested[i] |= mask;
              }
            }
  ...
  //std::cout<<"after openmp, the for use time["<<timelap<<"]\n"<<std::endl;
  /////////////////////
  //sleep(1);
  dTimerEnd();
